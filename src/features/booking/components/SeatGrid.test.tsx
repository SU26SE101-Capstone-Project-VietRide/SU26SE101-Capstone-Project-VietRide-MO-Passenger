...
jest.mock('react-i18next', () => ({
  ...
  useTranslation: () => ({
    t: (key: string, params?: { row?: string; column?: number; count?: number }) => {
      if (key === 'booking.seatMap.rowCode') return `Hàng ${params?.row}`;
      if (key === 'booking.seatMap.columnCode') return `Cột ${params?.column}`;
      if (key === 'booking.seatMap.selectedCount') return `Đã chọn (${params?.count})`;
...
const seatMap: SeatRow[] = [
  {
    ...
    columns: [1, 2],
    leftSeats: [
      { id: 'seat-01-1', label: '01-1', status: 'available', row: 1, col: 1, deck: 1 },
    ],
    rightSeats: [
      { id: 'seat-01-2', label: '01-2', status: 'available', row: 1, col: 2, deck: 1 },
    ],
  },
];
