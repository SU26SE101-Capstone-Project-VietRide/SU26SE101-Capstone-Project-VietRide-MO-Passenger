...
| --- | --- | --- |
| `UNIT_STATIC` | `PASS` | 2026-07-23 Expo 54 snapshot: `npx.cmd tsc --noEmit`, zero-warning ESLint and `git diff --check` pass; full Jest passes 76/76 suites and 407/407 tests; clean Android export bundles 4,542 modules and exits successfully. |
| `NATIVE_ANDROID_BUILD` | `PARTIAL_PASS` | Android prebuild previously regenerated successfully against Expo 54/RN 0.81 while retaining payment deep links, backup restrictions, blocked-permission removals and branded assets. On 2026-07-23 Gradle 8.14.3 config/dependencies passed with Android Studio JDK 21 and reached NDK compile; the current long workspace then failed at the known Windows 260-character Ninja object-path limit. A mapped-drive retry was rejected by Expo autolinking, so a current short physical-path compile/install and a key-enabled Maps build remain required. |
...
| M5 | P1 | Ticket and Parcel live trip tracking with native map | BE `origin/main@fcccf454` / `v1.40.0` Tracking gateway/controllers plus public Trip and Parcel DTOs | `IN_PROGRESS` | Auth, valid `tripId`, Parcel `dropoffStopId`, regional/Terms eligibility, restricted Google Maps keys, live tracking session | Socket.IO joins/rejoins the trip using bearer auth without query/log leakage; validates and scopes live GPS/ETA/status events; REST latest is bootstrap/fallback only; trail/ETA/status obey focus/network/AppState/terminal lifecycles; Ticket and Parcel reuse one panel; native map safely renders/caps trail and stop markers; camera follows without overriding user pan/pinch; Google attribution stays visible; consume additive destination/stop arrival fields without inventing lifecycle state; null/403/404 and missing IDs fail clearly | v1.40 fields remain preserved and one existing tracking panel/API/query owner is reused. The map now fits its first viewport after native readiness, detects pan/pinch before leaving follow mode, uses bounded camera animation, static markers and a contrast-safe native polyline. Native config conditionally pins Android Maps SDK 19.2.0, selects the latest renderer, registers usage attribution on both platforms, keeps keys out of JS and removes all Maps configuration from ineligible production builds. Focused map/config tests and static checks pass; regional eligibility, real Cloud credentials, key-enabled native compile and authenticated physical-device smoke remain open | 2026-07-23 |
